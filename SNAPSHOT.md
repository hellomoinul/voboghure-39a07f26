...
- ✅ **0.3** Google Photos link utility — **DONE 21 Aug.** `src/lib/media.ts` (URL kind detection: direct-image vs external-link vs invalid, https-only), `src/components/media/MediaImage.tsx` (renders direct URLs; external links get fallback card with "View on Google Photos"), `src/components/media/MediaLinkInput.tsx` (label + helper text + debounced live preview + clear button). Ready for F1/F4/S1 integration.
